...
window.SketchfabAPIClient = __WEBPACK_IMPORTED_MODULE_2_apis_viewer_lib_APIClient__["a" /* default */];
...
Sketchfab.prototype = {
    ...
    getEmbedURL: function getEmbedURL(urlid, options) {
        ...
        __WEBPACK_IMPORTED_MODULE_0_babel_runtime_core_js_object_keys___default()(this._transmitOptions).forEach(function (key) {
    ...
    reload: function reload(urlDomain) {
        ...
        s.setAttribute('src', 'https://static.' + urlDomain + '/api/sketchfab-viewer-' + this._version + '.js');
    ...
    _initializeAPIEmbed: function _initializeAPIEmbed(e) {
        ...
        this._client = new window.SketchfabAPIClient(this._target.contentWindow, this._apiId, domainSource);
...
APIClient.prototype = {
    ...
    listenServer: function listenServer() {
        ...
        var msgTypeFilter = [
        //'api.ready',
        'api.initialize.result', 'api.request.result', 'api.event'];

        window.addEventListener('message', function (e) {
            // filter message from everything
            // chrome extension, faye, other iframes
            if (e.origin !== this._domain || !e.data || !e.data.type || !e.data.instanceId) return;
