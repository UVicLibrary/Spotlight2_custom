    ...
    class IiifManifest
      ...
      def add_metadata
        ...
        sidecar.update(data: sidecar.data.merge(manifest_metadata))
          # sidecar.update(data: {"configured_fields" => sidecar.data.merge(manifest_metadata) })
      end
      ...
      def resources # returns array of IIIF::Presentation::ImageResource, NOT Spotlight resources
        @resources ||= sequences
                           .flat_map(&:canvases)
                           .flat_map(&:images)
                           .flat_map(&:resource)
